        ...
        if actual_mask is None:
            ...
            with col1:
                ...
                if traces_actual:
                    ...
                    st.plotly_chart(fig_actual, use_container_width=True, config={'responsive': True})
            ...
            with col2:
                ...
                if traces_baseline:
                    ...
                    st.plotly_chart(fig_baseline, use_container_width=True, config={'responsive': True})
            ...
            with col3:
                ...
                if traces_hybrid:
                    ...
                    st.plotly_chart(fig_hybrid, use_container_width=True, config={'responsive': True})
...
if patient_dir is None:
    ...
    if mask_path is None:
        ...
        if actual_mask is None:
            ...
            # Helper function to create figure with synchronized camera
            def create_figure(traces, scene_name, title):
                """Create figure with synchronized camera."""
                fig = go.Figure(data=traces)
                
                # All scenes use the same camera state
                layout_dict = {
                    scene_name: SCENE_LAYOUT,
                    "margin": dict(l=0, r=0, t=0, b=0),
                    "height": 600,
            ...
            with col1:
                ...
                if traces_actual:
                    fig_actual = create_figure(traces_actual, "scene1", "Actual")
                    st.plotly_chart(fig_actual, use_container_width=True, config=dict(responsive=True))
                    st.metric("Volume", f"{v_actual:.0f} mm³")
            ...
            with col2:
                ...
                if traces_baseline:
                    fig_baseline = create_figure(traces_baseline, "scene2", "Baseline")
                    st.plotly_chart(fig_baseline, use_container_width=True, config=dict(responsive=True))
                    mae_baseline = abs(v_actual - v_logistic)
                    st.metric("Volume", f"{v_logistic:.0f} mm³", f"MAE: {mae_baseline:.0f}")
            ...
            with col3:
                ...
                if traces_hybrid:
                    fig_hybrid = create_figure(traces_hybrid, "scene3", "Hybrid")
                    st.plotly_chart(fig_hybrid, use_container_width=True, config=dict(responsive=True))
                    mae_hybrid = abs(v_actual - v_hybrid)
                    improvement = (mae_baseline - mae_hybrid) / mae_baseline * 100 if mae_baseline > 0 else 0
                    st.metric("Volume", f"{v_hybrid:.0f} mm³", f"MAE: {mae_hybrid:.0f} ({improvement:+.1f}%)")
