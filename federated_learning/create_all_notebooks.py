...
class BrainTumorResNet(nn.Module):
    def __init__(self, n=4, pretrained=True):
        super().__init__()
        w = models.ResNet18_Weights.DEFAULT if pretrained else None
        self.model = models.resnet18(weights=w); self.model.fc = nn.Linear(self.model.fc.in_features, n)
    def forward(self, x): return self.model(x)

def create_model(n=4, device="cuda"):
    m = BrainTumorResNet(n).to(device)
    print(f"Model: ResNet-18 | Params: {sum(p.numel() for p in m.parameters()):,} | Classes: {n}"); return m
...
def generate_confusion_matrix(mp, tl, mname, device="cuda", save_path=None):
    model = BrainTumorResNet(NUM_CLASSES, pretrained=False).to(device)
...
BASE = "d:/Major_Project/FL_QPSO_FedAvg/Federated Learning QPSO"
