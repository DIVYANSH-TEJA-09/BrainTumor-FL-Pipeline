...
heterogeneous simulated clinical sites. Evaluated against FedAvg and FedProx
...
performance standard deviation of 1.56\% versus 2.27\% (FedProx) and 5.28\%
(FedAvg) under natural heterogeneity, and 3.65\% versus 6.05\% (FedProx)
and 12.82\% (FedAvg) under label skew. FedQPSO is the only algorithm that
maintains clinically useful accuracy ($\geq$80\%) for the weakest client at
convergence under label skew (80.00\% versus 77.50\% for FedProx and 60.42\%
for FedAvg). It also achieves the highest Glioma recall in both experimental
...
\subsection{Handling Non-IID Data in Federated Learning}
Li et al.\ \cite{li2020fedprox} proposed FedProx, adding a proximal term
$(\mu/2)\|w - w_{\text{global}}\|^2$ to the client loss to limit drift.
Karimireddy et al.\ \cite{karimireddy2020scaffold} introduced SCAFFOLD,
using control variates to reduce client drift variance. Zhao et al.\
\cite{zhao2018} analyzed accuracy degradation under label skew, confirming it
is the most challenging form of data heterogeneity. These approaches modify
\emph{client-side} training; FedQPSO instead redesigns \emph{server-side
aggregation} to be fitness-guided and fairness-aware.
...
FedProx \cite{li2020fedprox} modifies the client-side local training
objective to include a proximal regularization term:
\begin{equation}
    \mathcal{L}_{\text{FedProx}}(w_k) = \mathcal{L}_k(w_k)
    + \frac{\mu}{2}\,\|w_k - w_{\text{global}}\|^2,
    \label{eq:fedprox}
\end{equation}
where $\mu = 0.01$ penalizes the client for drifting too far from the
current global model. Server-side aggregation remains identical to FedAvg.
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s1_comparison.png}}
...
volatility than FedAvg and FedProx.}
\label{fig:curves_s1}
\end{figure}

\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s2_comparison.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s1_fairness.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s2_fairness.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s1_roc_auc.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{s2_roc_auc.png}}
\caption{ROC-AUC curves --- Setup 2. FedAvg Meningioma AUC drops to 0.970
under label skew; FedQPSO and FedProx maintain above 0.978.}
\label{fig:roc_s2}
\end{figure}
...
Right: FedQPSO. FedQPSO shows fewer Glioma misclassifications.}
\label{fig:cm_s1}
\end{figure}
...
Right: FedQPSO. FedAvg shows Meningioma/Glioma confusions from label-skew bias.}
\label{fig:cm_s2}
\end{figure}
